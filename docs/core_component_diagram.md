# AI Orchestrator — core/ 컴포넌트 다이어그램
...
flowchart TD
    ...
    subgraph core["core/"]
        ...
        subgraph nodes_pkg["nodes/"]
            ...
            subgraph te["task_executor.py"]
                ET["execute_tasks(state)\n① HITL: 플랜 실행 전 확인\n② Task 순차 실행\n   agent_id 있음 → AgentExecutor\n   agent_id 없음 → LLMGateway\n③ 최종 답변 생성 · StreamEvent 반환"]
            end
        end
    ...
    ET -->|"execute(agent_id, tool)"| AGT_EX
    ET -->|"interrupt() HITL"| LG_IT
    ET -->|"hitl_confirm_plan"| CMN_ST
